      ...
      <div className="container-max px-6 md:px-12 lg:px-24">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="flex flex-col leading-tight">
            ...
            <span className="font-outfit text-xs text-sage-green tracking-widest uppercase">
              Advocates
            </span>
          </div>
        ...
        <nav className="flex flex-col gap-6 px-6">
          {navLinks.map((link) => (
            <button
              key={link.href}
              onClick={() => handleNavClick(link.href)}
              className="font-outfit text-base text-white/80 hover:text-white text-left tracking-wide transition-colors duration-200 cursor-pointer"
            >
              {link.label}
            </button>
          ))}
